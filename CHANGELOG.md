...
**문자 작성팁 카테고리 추가** `applied 2026-09-18`
...
- 검증: 운영 DB 반영 완료. sitemap 에 /category/sms-tips 가 올라온 것과
  DB 의 이름이 `문자 보안`(공백 포함)인 것을 실제 조회로 확인했다

**(광고) 표시 의무 글 2편 추가** `applied 2026-09-18`
...
- 두 글은 서로 짝이라 상호 링크를 걸었다. guide.txt 16번에 따라 미발행 글로는
  링크하지 않으므로, 둘 다 발행된 것을 sitemap 으로 확인한 뒤에 넣었다
- 검증: 두 글 모두 발행 확인. 카테고리는 각각 `이메일 작성팁`과 `문자 작성팁`,
  태그는 주제에 맞게 `대량메일`/`대량문자` 로 갈렸다
