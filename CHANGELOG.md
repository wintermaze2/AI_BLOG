...
**문자 작성팁 카테고리 추가** `pending`
- 문자 글이 늘면서 보안과 발송 실무를 나눌 필요가 생겼다.
  `문자 작성팁`(sms-tips) 신설. 이메일 쪽의 보안/작성팁 구분과 같은 축이다
- 함께 **schema.sql 과 운영 DB 의 불일치를 수정**했다. DB 에는 `문자 보안`
  (공백 포함)으로 들어가 있는데 schema.sql 에만 `문자보안` 으로 적혀 있어,
  파일을 재실행하면 ON DUPLICATE KEY UPDATE 가 DB 의 이름을 덮어쓰는
  상태였다. 실제 발행 글의 카테고리 표시를 조회해 확인했다
- SEO.md §0 (2) 카테고리 표 갱신
- 카테고리 INSERT 는 **아직 운영 DB 에 적용 전**

**(광고) 표시 의무 글 2편 추가** `pending`
- email-ad-label-law, sms-ad-label-law. 정보통신망법 제50조·시행령 별표 6·
  제76조를 근거로 이메일 편과 문자 편을 같은 흐름으로 썼다
- 법률 내용이라 조문·시행일·과태료를 웹 검색으로 교차 확인했다.
  참조 섹션에 법제처와 KISA 1차 자료 링크를 달고 모두 HTTP 200 을 확인했다
- 문자 편의 핵심 차이는 **야간(21시~익일 08시) 전송에 별도 동의가 필요**
  하다는 점이다(법 제50조 제3항). 전자우편은 시행령이 정한 예외 매체라
  이 제한을 받지 않아, 같은 캠페인을 두 매체로 돌릴 때 사고가 나기 쉽다
- 두 글은 서로 짝이지만 **상호 링크는 아직 걸지 않았다.** guide.txt 16번에
  따라 미발행 글로는 링크하지 않는다. 둘 다 발행한 뒤 연결한다
...
**관리자 글 목록의 관리 열 행 높이 어긋남 수정** `applied 2026-08-31`
- 증상: 목록 마지막 "관리" 열만 높이가 낮아 행 아래 경계선이 잘려 보였다
- 원인: `.admin-table .col-actions` 에 `display:flex` 가 걸려 있었다.
  **td 에 flex 를 주면 그 셀이 display:table-cell 이 아니게 되어** 테이블
  레이아웃에서 빠진다. 행 높이 맞춤에 참여하지 않아 높이가 내용에 맞춰 줄고,
  border-bottom 이 다른 셀과 어긋난다
- 셀은 table-cell 로 두고 버튼을 inline 으로 배치했다. gap 은 flex 전용이라
  form 의 margin-left 로 대체하고 vertical-align:middle 로 정렬을 맞췄다.
  JSP 구조는 건드리지 않았다
- 같은 파일의 다른 display:flex 는 전부 div/ul/header 라 해당 없음을 확인
- 검증: mvn -o package 통과. 재배포 후 배포된 CSS 를 조회해 확인

### 콘텐츠

**TLS 글 추가** `applied 2026-08-31`
- what-is-email-tls: TLS 정의와 STARTTLS 동작 -> 왜 지금 필수 조건이 되었는지
  -> 메일링크의 TLS 우선 적용과 폴백
- TLS 를 SPF/DKIM/DMARC 와 혼동하지 않도록 "질문 축이 다르다"는 대비를 앞에 두고,
  구간 암호화이지 종단 간 암호화가 아니라는 점을 명시했다
- 발신자 요구사항은 웹 검색으로 확인했다. **TLS 를 명시적 요건으로 건 것은
  구글과 야후**이고(구글 공식 문서에서 "Use a TLS connection for transmitting
  email" 확인), 마이크로소프트의 2025년 5월 요구사항은 SPF/DKIM/DMARC 가
  핵심이며 TLS 는 권장이다. 제목도 "지메일이 요구하는" 에서 "구글·야후가
  요구하는" 으로 범위를 맞췄다
- 도입부 비유 수정: "엽서를 봉투에 넣는 일" 은 부정확했다. 봉투는 가리는 것이고
  TLS 는 읽어도 알아볼 수 없게 만드는 것이다. 글 뒤쪽의 "구간마다 복호화된다"
  설명과도 충돌했다(봉투는 끝까지 봉해진 채 가는 이미지). 암호 비유로 바꾸니
  "우체국마다 암호를 풀어 읽고 다시 암호로 바꿔 적는다" 로 이어진다
- 검증: 발행 완료. 카테고리 "이메일 보안", sitemap 반영 확인(글 14편).
  세로 흐름 도식과 비유 수정본이 실제 페이지에 반영된 것까지 조회로 확인했다.
  요약과 메타 설명은 관리자 입력 항목이라 저장소에 없다

**how-to-set-dns 의 DMARC 정책 안내를 메일링크 관점으로 수정** `applied 2026-08-31`
- 기존 "p=none 에서 멈추지 마세요" 는 일반론이라 메일링크 사용자에게 위험했다.
  SPF/DKIM 설정을 마치기 전에는 메일이 메일링크 도메인 기준으로 인증되어
  From 도메인과 정렬되지 않고, 그 상태에서 quarantine/reject 를 걸면
  **고객에게 보내는 정상 메일이 차단된다**
- "처음에는 p=none 으로 시작하세요" 로 바꾸고, 정책을 올릴 조건을 구체화했다
  (SPF·DKIM·리턴패스 등록 + 실제 메일의 dmarc=pass 확인)
- 처음 쓴 제목 "잘 모르겠으면 p=none 으로 두세요" 는 독자의 이해 부족을 전제로
  깔아 무시하는 느낌을 준다는 지적을 받아 위 문구로 바꿨다

### SEO

**구글·네이버 검색엔진 등록 완료** `applied 2026-08-31`
- 2026-08-15 에 준비를 마친 뒤 밀려 있던 항목. 소유확인과 사이트맵 제출 완료
- 확인: 홈에 google-site-verification 과 naver-site-verification meta 두 개가
  모두 출력되고, /sitemap.xml · /rss.xml · /robots.txt 가 전부 HTTP 200.
  sitemap 에 글 14편 포함(전체 67 URL)
- 색인까지는 며칠에서 몇 주 걸린다. Search Console 의
  **페이지 > 색인이 생성되지 않은 이유** 에서 진행을 본다.
  `/search` 와 결과 0건 아카이브는 의도적으로 noindex 이므로
  "noindex 태그에 의해 제외됨" 으로 잡히는 것이 정상이다

### 규약

**작성 규칙 세 가지 신설** `applied 2026-08-31`
- guide.txt [문체] 23번 — 쓰지 않을 표현 목록.
  "낡은/낡았다" -> "오래된/구형", "고치다" -> "수정하다".
  LLM 이 습관적으로 쓰는 말투라 한 단어만 눈에 띄어도 글 전체가 기계가 쓴 것처럼
  읽힌다. 지적받는 표현을 계속 쌓을 수 있도록 목록 구조로 만들었다
- guide.txt 14번 — 도식 작성 규칙. 아래 줄 기호를 위 줄의 특정 글자에 맞추지 말 것.
  한글은 폭이 넓어 폰트에 따라 어긋난다. TLS 글의 구간 암호화 도식에서 ↑ 가
  "중계 서버" 가 아니라 "[TLS]" 를 가리켜 설명이 정반대 뜻이 됐고, 피싱 글의
  표시 이름 예시에서도 같은 문제가 있었다. 세로 흐름으로 바꾸거나 표를 쓴다
- guide.txt 19번 — 맺음 CTA 에서 `p.cta-copy` 한 문장만 글 주제에 맞게 변경 허용.
  링크·버튼 문구·구조는 고정
...
- 이후 아이콘 전용 버튼으로 교체했다. 텍스트 라벨 대신 브랜드 로고만 있는
  원형 44px 버튼(터치 목표 최소 권장 크기). 로고 경로는 지어내지 않고 Simple
  Icons 에서 실제 SVG 를 받아 넣었고, 카카오만 사각 배지라 원형 버튼에 맞지
  않아 말풍선을 타원과 꼬리로 직접 구성했다
  - 아이콘 전용은 이름이 없으면 스크린리더에서 안 읽히므로 5개 모두
    aria-label 과 title 을 달고 svg 에 aria-hidden 을 줬다
  - share.js 의 복사 피드백도 함께 바꿨다. 버튼 textContent 를 "복사됨" 으로
    교체하는 방식이라 아이콘 전용에서는 SVG 를 지워버린다.
    role="status" aria-live="polite" 인 별도 알림 영역으로 옮겼다
...
**문자보안 카테고리와 스미싱 글 추가** `applied 2026-08-28`
...
- 검증: 카테고리 INSERT 운영 DB 반영 완료. sitemap 에 /category/sms-security 가
  올라온 것과 HTTP 200 을 실제 조회로 확인했다. 스미싱 글도 발행됨
...
**검색엔진 등록 준비 및 구조화 데이터 보강** `applied 2026-08-31`
